...
// array to store longitude/latitude for all markers
var LatLngList=[];
...
var SearchViewModel = function(){
  ...
  //store long/lat for all markers into array
 for (var i=0; i<initialMarkers.length; i++) {
  LatLngList.push(new google.maps.LatLng (initialMarkers[i].longitute,initialMarkers[i].latitude));
 }
  ...
  $.ajax({
      ...
      success: function(response){
        ...
        theArticles = response[1];
          ...
          var displayDetails = '<li class="article">' +
                   '<a href="' + url+ '">' +articleStr + '</a></li>'+
                   '<li>' + theSummary + '</li>';

           //display attraction name in the info window
           infowindow.setContent(displayDetails.toString());


          //open up info window
           infowindow.open(map, markerCurrent);

           //for smaller size window hide search menu while infoWindow is open and then re-display.
           if($(window).width() < 900){
